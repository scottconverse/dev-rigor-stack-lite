...
# glyphs (★ ✓ ▶) and turns every print into a hard crash. Degrade politely.
...
def cmd_next(a):
    ...
    if active:
        ...
        if not pending:
            print("rigor-goals: all stories complete ✓"); return
        g = pending[0]
...
def cmd_checkpoint(a):
    ...
    print("rigor-goals: all stories complete ✓" if not remaining else f"rigor-goals: {len(remaining)} stories left — continue with `next`.")
