...
description: Deep five-role audit across engineering, UI/UX, documentation, tests, and runtime QA, producing evidence-ranked findings, blast-radius analysis, a current-sprint punch list, and a next-sprint watchlist. Use when the user explicitly asks for a full/deep/multi-role audit or to "tear this apart," when the scope is a whole product/repo, or when work has a named Critical trigger such as auth/secrets, money, persisted data/migrations/deletion, security/privacy, concurrency/order/idempotency, install/deploy/rollback, irreversible operations, or broad public compatibility/API contracts. Do not trigger merely because routine Standard work is pre-merge, pre-release, customer-facing, or spans several files; use quick-audit-lite for that.
---
